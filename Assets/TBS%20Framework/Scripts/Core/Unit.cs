...
public abstract class Unit : MonoBehaviour
{
    ...
    /// <summary>
    /// Indicates the player that the unit belongs to. Should correspond with PlayerNumber variable on Player script.
    /// </summary>
    ...
    public virtual void DealDamage(Unit other)
    {
        ...
        other.Defend(this, PhysicalAttack);
    ...
    /// </summary>
	/*protected virtual void CriticalChance(int lak)
	{		
		int chance = Math.Floor((0.4*lak/1.4*lak))*100;
		int acak;
		...
		for(int i=0; i<chance; i++)
		{
			acak=rnd.Next(1,100);
			numberList.add(acak);
			int numberList = numberList.Count();
		}
    ...
    {
		//int ArmorModifier = 1-(int)(0.06*PhysicalDefense/(1+0.06*PhysicalDefense));
        MarkAsDefending(other);
        HitPoints -= Mathf.Clamp(damage - PhysicalDefense, 1, damage);  //Damage is calculated by subtracting attack factor of attacker and defence factor of defender. If result is below 1, it is set to 1.
